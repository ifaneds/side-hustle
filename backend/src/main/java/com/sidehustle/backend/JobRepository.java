...
public interface JobRepository extends JpaRepository<Job, Long> {
    ...
    @Query("SELECT j FROM Job j WHERE " +
           "(:generalSearch = '' OR :generalSearch IS NULL OR LOWER(j.title) LIKE LOWER(CONCAT('%', :generalSearch, '%')) OR LOWER(j.description) LIKE LOWER(CONCAT('%', :generalSearch, '%'))) AND " +
    ...
    List<Job> findJobsByFilters(
            @Param("generalSearch") String generalSearch, // Single parameter for general search
