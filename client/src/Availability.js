...
function Availability() {
  ...
  const handleEventSelect = (event, isSelected, isEditing = false) => {
    ...
    if (isSelected) {
      ...
      // Update events list
      if (isEditing) {
        setUnavailableEvents((prev) =>
          prev.map((e) => (e.id === event.id ? eventToAdd : e))
        );
      } else {
      ...
      if (eventToAdd.recurrence && eventToAdd.recurrence.type !== "none") {
        ...
        if (selectedRange) {
          ...
          getRecurringDates(eventToAdd, eventToAdd.startDate).forEach((date) =>
            affectedDates.add(date)
          );
        }
      } else if (selectedRange) {
        // Non-recurring range
        getDatesInRange(selectedRange.start, selectedRange.end).forEach(
          (date) => affectedDates.add(date)
        );
      } else {
      ...
      if (eventToRemove) {
          ...
          if (eventToRemove.startDate !== eventToRemove.endDate) {
            ...
            rangeDates.forEach((date) => {
              ...
              getRecurringDates(
                ...
                date
              ).forEach((d) => datesToRemove.add(d));
            });
          } else {
            // Single date recurring event
            getRecurringDates(eventToRemove, eventToRemove.startDate).forEach(
              (date) => datesToRemove.add(date)
            );
          }
        } else {
          // Non-recurring event
        ...
        setUnavailableDates((prev) =>
          prev.filter((date) => !datesToRemove.has(date))
        );
      }
  ...
  const saveAvailability = async () => {
    try {
      ...
      if (!userId) {
        ...
        alert(
          "You must be logged in to save availability. Please sign in and try again."
        );
        return;
      }
      ...
      const availabilityData = {
        ...
        unavailableEvents: unavailableEvents.map((event) => {
          ...
          return {
            ...
            title:
              event.title ||
              (event.start === "00:00" && event.end === "23:59"
                ? "All Day"
                : "Unavailable"),
          };
        }),
      ...
      const response = await fetch(`${API_BASE_URL}/api/availability/save`, {
        method: "POST",
        headers: {
    ...
    const loadAvailability = async () => {
      try {
        ...
        const events = data.map((event) => {
          ...
          const startDateTimeInUserZone = formatInTimeZone(
            startDateTime,
            userTimeZone,
            "yyyy-MM-dd'T'HH:mm:ssXXX"
          );
          const endDateTimeInUserZone = formatInTimeZone(
            endDateTime,
            userTimeZone,
            "yyyy-MM-dd'T'HH:mm:ssXXX"
          );
          ...
          const startTime = startDateTimeInUserZone.toLocaleTimeString(
            "en-US",
            {
              hour12: false,
              hour: "2-digit",
