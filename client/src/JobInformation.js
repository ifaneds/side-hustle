    ...
    const fetchJobTimeSlots = async () => {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/jobs/${id}/time-slots`
        );
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  useEffect(() => {
    if (timeSlots.length > 0 && unavailableEvents.length > 0) {
      const processedSlots = timeSlots.map((slot) => {
        ...
        const slotStartTime =
          slotStartDate.getHours().toString().padStart(2, "0") +
          ":" +
          slotStartDate.getMinutes().toString().padStart(2, "0");
        const slotEndTime =
          slotEndDate.getHours().toString().padStart(2, "0") +
          ":" +
        ...
        const slotEndMins =
          slotEndDate.getHours() * 60 + slotEndDate.getMinutes();
        ...
        const { clashing, nonClashing } = eventsOnThisDay.reduce(
          (acc, event) => {
            // Convert event times to minutes
            const [eventStartHour, eventStartMin] = event.start
              .split(":")
              .map(Number);
            const [eventEndHour, eventEndMin] = event.end
              .split(":")
              .map(Number);
            ...
            console.log(
              `with slot: ${slotStartTime}-${slotEndTime} (${slotStartMins}-${slotEndMins} minutes)`
            );
        ...
        console.log("\nFinal classification:", {
          ...
          clashingEvents: clashing.map(
            (e) => `${e.title}: ${e.start}-${e.end}`
          ),
          sameDayEvents: nonClashing.map(
            (e) => `${e.title}: ${e.start}-${e.end}`
          ),
        });
        ...
        {timeSlots.length > 0 ? (
          <ul className="time-slots-list">
            {timeSlots.map((slot) => {
              ...
              const clashingSlot = clashingSlots.find(
                (clash) => clash.id === slot.id
              );
